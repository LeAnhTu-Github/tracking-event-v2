...
interface AdsConfigTableProps<TData, TValue> {
  data: TData[];
  totalItems: number;
...
export function AdsConfigTable<TData, TValue>({
  data,
  totalItems,
  columns,
  isLoading,
  ...
  initialSorting,
  onReload
}: AdsConfigTableProps<TData, TValue>) {
    ...
    <div className='flex flex-1 flex-col space-y-4'>
      <DataTableToolbar table={table}>
        <div className='flex items-center gap-2'>
          ...
          <Button
            size='sm'
            onClick={() => router.push('/dashboard/config-ads/new')}
            className='h-8'
          >
