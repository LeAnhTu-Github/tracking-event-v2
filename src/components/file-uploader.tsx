...
export interface FileUploaderProps
  ...
  /**
   * Whether to show the file list after drops/uploads.
   * @type boolean
   * @default true
   * @example showFilesList={false}
   */
  showFilesList?: boolean;

  /**
   * Custom children to render inside the dropzone.
   * If provided, the default upload UI will be replaced.
   */
    ...
    <div className='relative flex flex-col gap-6 overflow-hidden'>
      ...
      {showFilesList && files?.length ? (
        <ScrollArea className='h-fit w-full px-3'>
          <div className='max-h-[520px] space-y-4'>
    ...
    <div className='relative flex flex-col gap-3'>
      {isFileWithPreview(file) && (
        <div className='flex w-full flex-col gap-2'>
          <div className='flex w-full justify-center'>
            <div className='bg-muted/30 relative inline-block max-h-[480px] max-w-full overflow-auto rounded-lg border p-2'>
              <img
                ref={imageRef}
                src={file.preview}
                alt={file.name}
                onLoad={handleImageLoad}
                className='h-auto max-h-[460px] w-auto max-w-full object-contain select-none'
                style={{ display: 'block' }}
              />
            </div>
          </div>
      ...
      <div className='flex items-center justify-between gap-4'>
        <div className='flex flex-1 flex-col gap-1'>
          ...
          {progress ? <Progress className='mt-1' value={progress} /> : null}
        </div>
        <Button
          type='button'
          variant='ghost'
